...
- **도메인 리로드가 꺼져 있어 `static` 값이 플레이 세션을 넘어 살아남는다.** 스모크 하네스가 바꾼
  `GameBalance` 프리셋·`RaceData` 값이 그대로 남아 이후 일반 플레이가 오염된다(FastTest가 남아 3초마다
  처형된 사례). 테스트용으로 정적 상태를 바꿨다면 **끝낼 때 반드시 원래대로 복원**하고, 이상 동작을
  의심할 때 `GameBalance.Current`부터 확인한다
