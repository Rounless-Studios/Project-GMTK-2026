...
### 2026-07-26 통합 개선 패스 검증

- Unity `6000.3.19f1` 배치 임포트와 스크립트 컴파일 성공
- EditMode 테스트 **141/141 통과**, 실패·스킵 0
- 레이스 차량/트래커 무결성 감시, 근접 순위 안정화, F8 진단 패널 추가
- 처형·CCTV·퀴즈·결과 화면 우선순위 조정 및 종횡비 대응 퀴즈 배치 추가
- 화면/오프스크린 처형 대상 마커, 라이벌 이름, 레이스 결과 통계 추가
- 오프트랙·전복·AI 정지 복구와 핸드브레이크 드리프트 안정화 추가
- Story/Normal/Brutal 난이도 프리셋과 최고 기록 고스트 리플레이 추가
- 충돌 스파크 풀링과 CCTV 렌더 비용 제한 추가

Play Mode의 실제 그리드 간격, UI safe-area, 차량 감각 및 WebGL 성능은 사용자 실행 확인이
남아 있으므로 관련 완료 체크박스는 올리지 않는다.
