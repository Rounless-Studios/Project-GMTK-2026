...
`Assets/GameBalance/Tests/Editor` 8파일 + `Assets/Quiz/Tests/Editor` 5파일 = **13개 테스트 파일 / 99개 테스트 케이스**

| 파일 | 케이스 | 파일 | 케이스 |
|---|---:|---|---:|
| `AiDrivingTests` | 14 | `OvertakeChallengeStateTests` | 8 |
| `CurseCooldownStateTests` | 14 | `AiPersonalityRosterTests` | 6 |
| `BoostStateTests` | 12 | `QuizQuestionTests` | 5 |
| `GameBalanceSettingsTests` | 11 | `FallRespawnStateTests` | 4 |
| `DurabilityStateTests` | 10 | `WorldSpaceQuizCanvasFollowerTests` | 3 |
| `QuizSessionControllerTests` | 8 | `ButtonMashProgressTests` | 2 |
...
1. **`FinalGate`에 씬 트리거가 없다.** `ReportGateCrossing()`의 유일한 호출자가 `GMTKAutoPlaytest`뿐이라
   실제 플레이에서는 관문을 통과해도 아무 일도 일어나지 않고, 20초 뒤 `duelCars[0]`(생존자 중 낮은
   레이스 인덱스)이 자동 승리한다. P0 승리 조건이 성립하지 않는다.
...
3. **`RaceHud`가 어디에도 배치되지 않았다.** 처형 카운트다운·최하위 경고·부스트·저주·추월·관문
   상태가 화면에 전혀 표시되지 않는다.
4. **`GameJamDefault.elimination.intervalSeconds = 10`이고 `warningSeconds`와 같다.** GDD 기준값 30이
   아니며, `Validate()`가 오류를 뱉는 상태다.
5. **`GameJamDefault.maximumDurability = 1000000`** — 대파를 미루기 위한 임시값.
6. **추월 실패 속박이 실제로 감속하지 않는다** (`BindSpeedMultiplier` 소비자 없음).
7. **저주 확정 구성이 아직 코드에 없다.** 방어형 흐름 자체는 확정·구현이지만, AI가 표적을 연속으로
   찍는 것을 막는 유예(`hostileEffectGraceSeconds`)와 "표적이 됐다"는 사전 신호가 없다.
8. ~~AI 성격 구성이 확정안과 다르다~~ → **2026-07-26 해결.** 구성이 `AISettings.personalityAssignments`로
   옮겨져 프리셋이 정한다. `AIArchetype`/`AIArchetypeProfile`은 제거했다.
   남은 조치: **프리셋 에셋 2종에 폐기된 `archetypeAssignments`·`profiles` 키가 아직 남아 있다.**
   필드명이 바뀌었으므로 Unity는 이 키를 무시하고 코드 기본값(= 확정값)을 쓴다. 동작은 정상이지만
   Unity에서 두 에셋을 한 번 재저장해 죽은 키를 지워야 한다 (`.asset` YAML 손편집 금지 규칙 때문에
   에이전트가 정리하지 않았다).
9. `Assets/GameBalance/Resources/`(프리셋 2종)와 `ExecutionCctvDirector.cs`가 **Git 미추적**이다.
   구 위치 `Assets/Resources/GameBalance/`는 삭제 상태이므로, 지금 커밋하면 프리셋 없는 상태가 푸시된다.
10. `BoostController`가 레거시 `Input` API를 사용한다 (프로젝트 규칙 위반, Active Input Handling 의존).
11. RCC 차량 렌즈 플레어가 URP에서 표시되지 않음 (`LegacyLensFlareUrpBridge` 미검증).
12. 레거시 랜덤 이벤트가 `GMTK_Race`에서 켜져 있다 (`enableEvents: 1`).
